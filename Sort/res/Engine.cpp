...
Engine::Engine(Coord winSize, int maxNum, int AlgoNum, const char* winTitle)
    : m_winSize(winSize), m_algoID(AlgoNum), m_numbers{ genRand(maxNum, winSize.Y) }
{
    calcNumbers();
...
{
    // Calculate maximum column width based on window size
   ...
   m_columnWidth = static_cast<float>(m_winSize.X) / n;
    ...
    if (m_maxValue > m_winSize.Y)
    {
        // Scale down the maximum value if it exceeds window height
        m_maxValue = m_winSize.Y;
    }
...
std::vector<int> Engine::genRand(int maxNum, int winH)
{
    ...
    {
        num[i] = std::uniform_int_distribution<int>(0, maxNum - 1)(rng);
    }
